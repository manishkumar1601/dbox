"""`phpbox doctor` — environment health checks."""
...
from phpbox import detection, engine, plugins
from phpbox.config import SUPPORTED_PHP, ProjectConfig
...
def run_checks(project_dir: Path, cfg: ProjectConfig | None) -> list[Check]:
    ...
    if cfg is None:
        checks.append(Check("phpbox.yml present", False, "run `phpbox init`"))
        return checks
    checks.append(Check("phpbox.yml present", True))
    ...
    if daemon_ok and engine.compose_file(project_dir).exists():
        ...
        checks.append(
            Check(
                "Containers running",
                bool(running),
                ", ".join(running) if running else "run `phpbox start`",
            )
        )
