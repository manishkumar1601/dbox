...
DBox shells out to ``docker compose`` rather than the Docker SDK so it works
...
from dbox.config import DBOX_DIR
...
def run_once(
    ...
    A global ``dbox-composer-cache`` volume is mounted so Composer downloads
    are reused across every project — making repeat ``dbox create`` runs much
    ...
    args = [
        "run", "--rm", "--no-deps",
        "-v", "dbox-composer-cache:/tmp/dbox-composer",
        "-e", "COMPOSER_CACHE_DIR=/tmp/dbox-composer",
    ]
