"""Render the ``.dbox/`` Docker artifacts from a ``ProjectConfig``.

Everything under ``.dbox/`` is disposable and regenerated on demand, so the
...
from dbox import extensions, plugins
from dbox.config import DBOX_DIR, ProjectConfig
...
def generate(project_dir: Path, cfg: ProjectConfig) -> Path:
    """Write the full ``.dbox/`` tree and return its path."""
    ctx = _context(cfg)
    base = project_dir / DBOX_DIR
