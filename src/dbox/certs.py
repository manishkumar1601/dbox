"""Local TLS certificate generation for `dbox ssl`.
...
def certs_dir(project_dir: Path) -> Path:
    return project_dir / DBOX_DIR / "certs"
...
def _self_signed(project_dir: Path, host: str) -> bool:
    ...
    if engine.build(project_dir, "php") != 0:
        return False
    script = _PHP_SELF_SIGNED.format(dbox=DBOX_DIR, host=host)
