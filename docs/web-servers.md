...
Switch with `dbox server <type>` (then `dbox start`), or set `server.type`
in `dbox.yml`. Four servers are supported.
...
to the PHP-FPM container. Config is generated at `.dbox/nginx/default.conf`
...
container. DBox enables `mod_rewrite` and `mod_ssl`, sets the `DocumentRoot`,
...
is generated at `.dbox/apache/ssl.conf` and mounted into the container.

## OpenLiteSpeed

`litespeedtech/openlitespeed` runs as a separate `web` container and connects to
the PHP-FPM container over FastCGI. DBox generates two configs:

* `.dbox/litespeed/httpd_config.conf` — server config: the listener(s), the
  external FastCGI processor, and the script handler mapping `.php`.
* `.dbox/litespeed/vhconf.conf` — the virtual host: document root, index
...
All four servers support HTTPS via `dbox ssl enable`. Certificates are
generated automatically and mounted at `/etc/dbox/certs/` (nginx, Caddy,
